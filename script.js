...
async function fetchPipedStream(videoId, attempt) {
	...
	try {
		const url = getPipedApi() + "/streams/" + videoId;
		const response = await fetch(url);
		if (!response.ok) {
		...
		console.log("Piped instance failed:", getPipedApi(), error);
		rotatePipedInstance();
...
async function loadVideoIntoPlayer(video, addToHistory, scrollToPlayer, autoplay) {
	...
	try {
		...
		if (!stream) {
			showPlayerStatus("No playable stream found. Try Open on YouTube.");
			return;
		}
		if (stream.type === "hls") {
			if (videoPlayer.canPlayType("application/vnd.apple.mpegurl")) {
				...
				showPlayerStatus("HLS not supported on this browser.");
				return;
			}
		} else {
			videoPlayer.src = stream.url;
		...
		showPlayerStatus("Could not load this video. Try Open on YouTube.");
	}
}
